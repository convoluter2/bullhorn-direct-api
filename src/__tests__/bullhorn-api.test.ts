...
describe('BullhornAPI', () => {
  ...
  describe('getAuthorizationUrl', () => {
    it('should generate correct authorization URL without username/password', () => {
      const url = api.getAuthorizationUrl('client-id', 'https://redirect.uri', 'state-123')
      expect(url).toContain('client_id=client-id')
      expect(url).toContain('response_type=code')
      expect(url).toContain('state=state-123')
      expect(url).toContain('redirect_uri=https%3A%2F%2Fredirect.uri')
      expect(url).not.toContain('username')
      expect(url).not.toContain('password')
    })

    it('should include username and password when provided', () => {
      const url = api.getAuthorizationUrl('client-id', 'https://redirect.uri', 'state-123', 'user@test.com', 'pass123')

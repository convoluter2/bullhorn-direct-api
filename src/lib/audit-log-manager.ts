...
export class AuditLogManag
    ...
    const sanitized = { ...log }
          ...
          _truncated: true,
          _summary: this.createDetails
   

      const rollbackStr = JSON.stringify(sanitized.rollba
        sanitized.rollbackData =

        sanitized.details._r
      }

      if (sanitized.failedOpe
        if (!sanitized.deta
        }
      }
  ...
  pri
    ...
    if (details.recordCount !== undefined) summary.push(`Records
    if (details.errorCount !== undefine
